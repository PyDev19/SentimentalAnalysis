...
class PositionalEncoding(nn.Module):
    def __init__(self, dimensions: int, max_seq_length: int):
        ...
        pe = torch.zeros(max_seq_length, dimensions) # initialize the positional encoding matrix
        ...
        pe[:, 0::2] = torch.sin(position * div_term) # fill the even indices with the sine of the position multiplied by div_term
        pe[:, 1::2] = torch.cos(position * div_term) # fill the odd indices with the cosine of the position multiplied by div_term
